...
>
> **Why byte-for-byte matters:** this sheet is signed by a citizen and filed as a government paper, so what prints has to be the office's own file, not a re-encoding of it. `GET /template-types/` gained a `blank_form` flag so the screen words itself for both without keeping its own copy of the split (UC-008), and the preview dialog gained explicit **Print** and **Download** — printing from here is the whole purpose of the entry, and the PDF viewer's own download would name it after the blob id (UC-058).
>
