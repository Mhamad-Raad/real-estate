...
| **Store layout** | `<CATEGORY>/<client_id>_<pid>/<category>_<institute>_<person>_<type>__<id>.pdf` (§6.7) | **`<CATEGORY>/<pid>/<institute>_<type>__<id>.pdf`** — the folder is keyed by the **PID**, so one human keeps one folder even if their record was re-entered; the on-disk name drops what the folders already say. `display_filename` still carries the long self-describing name for downloads | — (settled It.5) |
