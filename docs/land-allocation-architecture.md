...
| **Release 1.7.0 (build 11)** (2026-09-23) | `VERSION` = 1.6.0 / build 10 (the office's build since 2026-09-08) | **`VERSION` = 1.7.0 / build 11.** UC-125 (the card camera takes a still, sharp card by itself — the *Take* button stays), UC-126 (the card reader rebuilt against a bench of real cards — ID number and birth date read right far more often) and UC-127 (name boxes refuse a digit, at both doors and on scan-confirm) — no migration, no new job, no new env var (`OCR_TESSDATA_DIR` unset = the bundled models). `UPDATE.txt`'s "what is new" is rewritten for the three | New user-visible behaviour, so a minor bump. UC-125's thresholds ship uncalibrated on the office's webcam on purpose: the manual shutter still works, and the office's camera is what calibrates them |
| **A name box refuses a number** (2026-09-22, the office — UC-127) | §4.1 filters the *numeric* boxes as they are typed (`lib/pid.ts`, `lib/phone.ts`); a name box took anything | The four name fields of the intake form and of the backlog form — the beneficiary's own name, their mother's, and both spouse names — **drop a digit at the keystroke** (`lib/name.ts` over `digits.withoutDigits`). Arabic-Indic and Persian digits count as digits; nothing else is touched, so a hyphen, an apostrophe or a diacritic in a real name survives. Box-only, like the phone field before its validator. **Review, same day:** the rule also covers the **scan-confirm** door (§6.4) — the one the office actually uses — and the caret is held in place by `lib/caret.ts`, since a refused keystroke otherwise makes React restore the value and drop the cursor at the end of the name. See **§4.1** | A number typed into a name is caught by nobody afterwards — it is copied onto the generated letter and printed. Digits only, not "letters only": names here legitimately carry punctuation, and a stricter gate would refuse real names |
| **The card reader measured and rebuilt against real cards** (2026-09-17, the user — UC-126) | §6.2 reads each side of the uploaded file whole (Arabic + `eng`), retries a sheet framed only when the first pass found nothing, takes the PID from the front and the MRZ parse of the Latin pass, and maps the name block purely by line position | **Scored first:** `manage.py ocr_bench <manifest.csv>` runs cards with known values through the real upload path and reports each field as correct / empty / **wrong** (a wrong value is the dangerous outcome). **Then:** a scanned sheet has its card found by colour and *physical size* and straightened (`ocr/card_geometry.py`); the card number and the MRZ are also read from their **fixed zones**; the MRZ is parsed from **every** read (zone `eng` + zone `Arabic` + both whole-side passes) and the most check-digit-verified parse wins; the card number is **voted** across all reads, a front/MRZ agreement is verified, and a verified birth year both picks and checks it (the number starts with the year; one leading-digit misread is repaired only when the year vouches for it); the **century** of the MRZ birth date comes from the card number (`000701` on a `1900…` card is 1900); the filler-free line 2 of the older layout is recognised; the name block is **anchored on legible printed labels** as whole words, so a blank surname row keeps its place. See **§6.2** | On 19 real cards (6 office flatbed scans, 13 photos) the old reader got the card number right on 11 with 3 wrong, the birth date on 1 of 12 with 1 confidently wrong, no full or mother's name right, and took ~14 s per scan. Now: card number 19/19, birth date 9/12 with none wrong, names 1 and 2 right with wrong names down from 10+3 to 4+2, ~10 s per scan. The larger `tessdata_best` models and EasyOCR were measured and **not** adopted (below) |
...
| **A file preview is one page tall** (2026-09-06, the office — UC-123) | §5.9's preview and `DocumentPreview` give the PDF a fixed **28 rem** frame | **`features/documents/PageFrame`**, shared by both: an iframe at the page's own ratio (A4 plus the viewer's toolbar, `aspect-[2/3]`) of whatever width it gets, **capped at 75 vh** so a wide screen never gets a frame taller than the window it scrolls in. The backlog door's image preview takes the same cap. See **§8.3**, **§5.9** | The browser's viewer fits the page to the frame's width, so a fixed height showed roughly half a page and asked the office to scroll inside the frame to read the rest of the very scan they were checking. The first cut had no cap and ran off the screen — the office asked for less |
| **The backlog door shows the picked file back** (2026-09-02, the office — UC-120) | §5.9's screen takes one PDF; nothing shows it back beyond the filename in the native input | An inline preview under the input, from a **local object URL** — no request, nothing stored: a PDF in an iframe (the browser's own viewer, offline), an image in `<img>`, a scanner TIFF a named no-preview note. Filename + size above it, bidi-isolated. See **§5.9** | A backlog case is filed under **one** PDF picked from a desk of look-alike scans, and that scan is the case's only copy (§10.3) — a wrong pick is exactly the mistake this screen invites, and the cheapest place to catch it is before Save |
| **A date box refuses a number it can never hold** (2026-09-02, the office — UC-121) | §9's `DateField` caps each box's **length**, not its value: a 39 sits in the day box until the field-level revert quietly restores the stored date on blur | **`segmentMax`** (`lib/date.ts`) owns every box's cap — day ≤ the real month length once the month (and, for February, the year) is typed, else 31 (29 for an open February); month ≤ 12; year ≤ the window's top — and an over-max keystroke is refused outright. A day a **later-typed** month cannot hold is **clamped to that month's last day** (31 → February → 29 → a non-leap year → 28). `segmentIsFinished` and `stepSegment` take their maxima from the same function. See **§9** | The office typed 3x into the day box and watched it accepted, then silently undone — the accepted-then-reverted shape of UC-072, one layer up. The cross-box conflict **clamps** rather than refuses because the day is typed *first* in this field, so 31-then-February is a legitimate order and wiping the whole date would throw away everything typed |
...
| **Release 1.5.0 (build 8)** (2026-09-01) | §0's row below: `VERSION` = 1.5.0 / build 7, bundle built, verified and smoke-booted | **`VERSION` = 1.5.0 / build 8.** Build 7 plus UC-119 (a list row opens its detail) and its review fix — frontend only, no migration. The user tested 1.5.0 in the browser first ("tested and works fine") and this is the cut that goes to the office | Build 7 never left the Mac, but it exists as a verified file, and two images answering the same number is what the stamp prevents. Same minor version: nothing changed in behaviour the office was told about, only how a row is reached |
| **A list row is the way into its detail** (2026-09-01, the user — UC-119) | §8's list screens reach a detail through a link in one cell (the case name) or a button at the row's end (the client's cases) | **`features/common/LinkRow`** — a `TableRow` made focusable (`tabIndex`, the ordinary grid pattern), a pointer cursor, and navigation on click or Enter. **No `role`, no `aria-label`**: a `<tr>` re-cast as a link loses its cells for a screen reader, and a label would replace the row's own content when read aloud (found in review). Used by the Processes and Clients lists. Controls inside the row keep their own click (anything under `a, button, input, label, [role=button], [role=menuitem]` is left alone), a text selection is not a click, and Enter counts only when the row itself is focused. The in-cell link stays for middle-click/new-tab. Activities already opened on row click; its rows are the same pattern by hand | The office had to find the one clickable word or button in a row, and it differed by page. One primitive so every list behaves the same and the guards live in one place |
| **Release 1.5.0 (build 7)** (2026-09-01) | §0's row above: `VERSION` = 1.4.0 / build 6, bundle built and rehearsed, office on 1.3.0 (build 4) | **`VERSION` = 1.5.0 / build 7.** Carries everything in build 6 plus UC-118 (the compiled case is produced, not kept), its review, and the scanned-case-file guard. `UPDATE.txt` gains job 3, `retire_compiled_exports`. **No new migration** — `makemigrations --check` clean, so the build-6 upgrade rehearsal still describes the schema path | The build-6 bundle never left the Mac, but it was cut and verified as a *file*, and a second image answering "build 6" with different code is exactly what the stamp exists to prevent — so 7. Minor bump because the office-visible behaviour changed (a file they used to find on the case is now produced on demand), not because anything in the data changed |
| **The compiled case is produced, not kept** (2026-08-31, the user — UC-118) | §10.3 stores the export as a `Document(input_source=system_generated)` on the case; §6.8 names it the one generated output that stays a real Document | **A one-read job file under `GENERATED_ROOT/compiled/`**, exactly like the Step-1 letter: previewed and printed off the mark-complete press, collected on its first read, swept by `_discard_stale_output` like the rest. Nothing is filed on the case. **`retire_compiled_exports`** removes the copies stored before this (row soft-deleted, audited `file_removed`, PDF unlinked); `supersede_generated_documents` now filters on `input_source` so a **scanned** `CompiledCase` — the backlog door's only copy of a paper case (UC-114) — is never touched, and `belongs_in_compilation` merges that scan rather than excluding it by type. See **§10.3**, **§6.8** | The export is every paper on the case merged again, so a closed case cost roughly **twice** its papers on disk — for a file that can be produced again from the same papers in seconds. The office is about to carry thousands of backlog cases in and asked for the space. What is given up: the exact PDF printed on the day; a case amended after closing compiles to today's data, and the audit row still says an export existed |
...
| **Nginx must pass a whole case file** (2026-08-30, It.9 — found reviewing UC-114) | §2.3's proxy carries `client_max_body_size 64m`, sized for the 25 MB single-paper limit with headroom | **`client_max_body_size 210m`**, covering `MAX_GENERATED_BYTES` (200 MB) — the bound `size_limit_for` gives a compiled case however it was made. Nginx stays deliberately permissive; the app decides, and an ordinary paper is still held to 25 MB by Django | The backlog door takes a scan of an **entire case file** as one upload. Between 64 MB and 200 MB — a twenty-page colour scan at 300 dpi — the request died at the **proxy** with a bare 413 and nothing in the app's log to explain it, on the one machine nobody can debug. Neither the test suite nor a dev run could see it: dev has no Nginx in front of Django at all |
| **A fresh install pins `COMPOSE_PROJECT_NAME` from day one** (2026-08-30, It.9) | §2.6 and the install sheet leave Compose to name the project after the folder it was started from | **`COMPOSE_PROJECT_NAME=landalloc` ships filled in inside `.env.example`**, and `install.md` tells the reader to leave it alone and why. The office pinned it by hand mid-update on 2026-08-19; a machine installed from this bundle onward has it from the first `up` | The folder is named for the version it shipped as (`landalloc-1.4.0-build6`), so it changes with every update — and Compose started from the new folder looks for a project that never existed and brings the app up **empty**, with the real database untouched under the old name. Pinning it is also what makes the versioned folder name harmless, so `build-bundle.sh` keeps it: on the drive it is the one thing that says which build you are holding |
| **Building a bundle needs a real VM** (2026-08-30, It.9) | §2.3/§12 describe the cross-build for `linux/amd64`; nothing says what the build machine needs | **Colima: at least 6 CPUs, 12 GB, and `--vz-rosetta`.** With the 2 CPU / 2 GB default and QEMU emulation, `npm ci` in the frontend stage dies with **`Segmentation fault (core dumped)`, exit 139** — the build fails halfway with no bundle. Recorded here because it is invisible until you try | The failure names neither memory nor emulation, so it reads as a broken lockfile. Rosetta translates x86-64 natively rather than interpreting it, which is both far faster and what stops `npm` crashing. Also: run the script as the **last** command of anything wrapping it — piping its output to a log and ending with `tail` reports the *tail's* exit code, and a failed build looks like a clean one |
...
| **A taken national ID says whose it is — and is found however it was typed** (2026-08-31 — UC-117) | §4.1: `pid` uniqueness is left to the `UniqueValidator` DRF generates from `ix_client_pid_active` | **`PidTakenValidator`** replaces it — it may not simply be dropped, being the **only** uniqueness check on the client-edit path, which has no service of its own. Both it and `assert_pid_is_free` ask **`clients.selectors.pid_holder`**, one query that normalises first, and both answer **`errors.pid.taken:<holder>`** built by `common.validators.pid_taken`. `lib/apiError.ts` splits the name off and interpolates it **bidi-isolated** (§9). See **§4.1** | Two defects in one place. The office read `"client with this pid already exists."`: English on otherwise-Sorani screens, naming the *column*, never saying **who** held the ID — the one fact the person typing needs; the good message sat in `clients/services.py`, unreachable behind the generated validator. And **DRF runs a field's validators before `validate_pid` folds Arabic-Indic digits**, so `١٩٧٧…` never matched a stored `1977…`: the serializer passed and the write hit the index as an IntegrityError — **an HTTP 500 for a lawyer typing digits the way this office writes them** (§9), on the edit path where no service check stands behind the serializer. Both close by asking one normalising query. **The one message carrying a runtime value**: the rest stay parameterless because their bounds are constants, and a holder's name is not |
| **Both doors that create a beneficiary share one duplicate gate** (2026-08-30, the office — UC-116) | §5.7's pre-save check lives inside `ProcessCreatePage`; the backlog form (§5.9) shipped without it | **`clients/useDuplicateGate`** holds the check and its dialog, and both screens use it — a PID or household match can only be cancelled, a similar mother's name is advisory. **Every dialog button is now explicitly `type="button"`** (`DuplicateWarningDialog`, `ConfirmDialog`, the dialog's own ×). See **§5.9** | The warning mattered more on the backlog form than on the intake one: `similar_name_flagged` is surfaced inside **Step 1's panel**, and a backlog case has an empty Step 1 nobody opens — so it was recorded and unreachable. Sharing the gate rather than copying it means a person refused at one door cannot walk in the other. The button types are not tidiness: `Button` sets **no default `type`**, so an untyped button inside a `<form>` submits it — the dialog renders inside the backlog form, and **clicking Cancel saved the case**. Worse, the test that should have caught it passed for the wrong reason, seeing a save that came from the form submitting rather than from the proceed button; only the paired cancel test exposed it |
...
- **Validation messages are i18n keys, not sentences** (2026-08-10). The domain validators answer with `errors.<area>.<case>` and the frontend renders the language, because a rejected field is the most routine thing a user meets and the office reads these screens in Sorani (§9). Same precedent as the institute and document-type vocabularies, and guarded the same way: `common/test_validation_keys.py` fails if a key has no translation, so one can never surface as a raw dotted string. **DRF's own built-in messages** ("This field is required.", "Date has wrong format…") remain English and are shown verbatim — translating the framework's whole catalogue, including a `ckb` locale it does not ship, is out of scope; `translateApiMessage` passes anything that is not an `errors.*` key straight through.
  - **One key may carry one runtime value: `errors.pid.taken:<full name>`** (2026-08-31, UC-117). The rest stay **parameterless on purpose** — their bounds are constants (`10–11`, `1900`) and belong in the translation rather than travelling through DRF's string-only error shape. A national ID's current holder is not a constant, and naming them is the entire value of the message: "already exists" leaves the lawyer to go and search for who. `translateApiMessage` splits on the first colon after a well-formed key, interpolates as `{{name}}`, and **wraps the value in a bidi isolate** — a Latin name inside a Sorani sentence reorders the words around it otherwise (§9). Build the message with `common.validators.pid_taken`, never by hand, so the shape has one owner.
...
- **A national ID already on file names its holder** — `errors.pid.taken:<full name>`, the one validation message that carries a runtime value (UC-117). Enforced by `PidTakenValidator` on the serializer and by `assert_pid_is_free` in the service layer, both asking the **same** query, `clients.selectors.pid_holder`, so the client form, the scan confirm and the backlog form can never disagree about who holds an ID. **That query normalises before it looks**, which is not a detail: DRF runs field validators *before* `validate_pid` folds Arabic-Indic digits, so a check on the raw input missed `١٩٧٧…` against a stored `1977…` and let the write reach the index as a 500. The frontend splits the name off and **bidi-isolates** it, because a Latin name inside a Sorani sentence reorders the words around it otherwise (§9).
...
- **A name box refuses a digit as you type** (2026-09-22, the office — UC-127): the beneficiary's name, their mother's name and both spouse names take no number, in **any** script — `lib/name.ts` drops Arabic-Indic and Persian digits as readily as ASCII ones, over the one helper that knows what a digit is (`digits.withoutDigits`). Unlike the phone and ID boxes this one has **no validator behind it**: it is a keystroke convenience and nothing on the server refuses a digit in a name, so a name already in the database is untouched. It is applied at **both doors that create a beneficiary** — the intake/backlog forms and the **scan-confirm review** (§6.4), where the card's number is already filtered the same way; the OCR *draft* is deliberately **not** filtered, because a digit the engine misread off a card is the thing the lawyer is there to see and correct. Only digits are dropped — a name may carry a hyphen, an apostrophe, a full stop or a diacritic, and "letters only" would refuse real names.
  - **A filtered box has to hold its caret** (`lib/caret.ts`, measured 2026-09-22). A controlled input whose `onChange` refuses a character ends up holding a value React did not render, so React puts its own back — and that assignment drops the cursor at the **end of the field**. Typing a digit into the middle of a twelve-character name moved the caret from 6 to 12, so the next letter landed at the end of the name. `filterKeepingCaret` returns it — and it **counts** the surviving characters before the caret rather than subtracting the dropped ones, which is the only form that survives a filter that also trims the end: with plain arithmetic, typing into the middle of a full 12-digit ID left the caret *before* the digit just typed and the next two came out transposed (second review, same day). It does nothing at all when a keystroke is accepted, so an ordinary letter — or an active IME composition — is never touched. **The intake form's own ID and phone boxes still have the original caret jump** — they are short, LTR and rarely edited mid-string, so it was left rather than changed under a shipped screen.
...
| **Compiled case** (§10.3) | `POST /api/v1/processes/{id}/compile/` | Step-5 export: summary cover sheet + every document, merged; returns a job → `202`, and the file is read once from `GET /generation-jobs/{id}/file/` like the letters — **nothing is filed on the case** (UC-118). **No completeness gate server-side** — "only after the case closes" is a UI rule (§10.3), deliberately, because an export of an open case is readable output and not a privileged act | Assignee or Admin |
...
- **The duplicate warning is the intake form's own** (the office, 2026-08-30 — UC-116). Both screens that
...
- **The picked file is shown back before saving** (the office, 2026-09-02 — UC-120). An inline
  preview under the input, from a local object URL — a PDF in the browser's own viewer, an image
  as an image, a scanner TIFF (which no browser renders) a named note with the filename and size
  still there to check. The scan is the case's **only** copy, picked from a desk of look-alike
  files — the cheapest place to catch a wrong pick is before Save. The frame is the shared
  `PageFrame` (§8.3, UC-123): one page tall, capped to the window.
...
> **Measured and rebuilt (2026-09-17 — UC-126).** The reader is now judged by a bench, not by eye:
> `python manage.py ocr_bench <truth.csv>` runs cards with known values through exactly the upload
> path (`normalise_to_pdf` → merge → `read_card`) and reports each field as **correct, empty or
> wrong**, keeping wrong apart because a wrong pre-filled value invites acceptance (§6.5). The
> manifest and samples are real cards and live outside the repository.
>
> | 19 real cards (6 flatbed, 13 photo) | card number | birth date (12 with a back) | full name | mother's name | per scan |
> |---|---|---|---|---|---|
> | before | 11 right, 3 wrong | 1 right, 2 wrong (1 verified) | 0 right, 10 wrong | 0 right, 3 wrong | ~14 s |
> | after | **19 right, 0 wrong** | **9 right, 0 wrong** | 1 right, 4 wrong | 2 right, 2 wrong | ~10 s |
>
> What changed, each kept only because the bench moved: **(1)** a scanned sheet has its card found
> — the only blue object on white paper, eroded apart from anything it touches until a piece of the
> card's *physical* size (known from the page's pixel size) separates — and straightened to
> 1712×1080 (`ocr/card_geometry.py`); a photo or capture is already the card and is untouched.
> **(2)** The card number and the MRZ are also read from their fixed zones on that card. **(3)** The
> MRZ is parsed from every read and the parse with the most verified check digits wins
> (`mrz.parse_best`): on some cards the Arabic model read the zone cleanly while `eng` produced
> noise. **(4)** The card number is **voted** across the zone read, both whole-side passes and every
> MRZ read; exact 12-digit reads outweigh windows cut from a longer corrupted run; a number the
> front and MRZ agree on is verified; and because the number starts with the birth year, a
> check-digit-verified birth date picks between candidates, flags a number that does not match,
> and repairs only a misread *leading* digit (`497…` → `197…` for a 1971 birth). **(5)** The MRZ's
> two-digit year takes its century from that number. **(6)** A filler-free MRZ line 2 (older
> layout, national ID printed there) is recognised by its shape. **(7)** The name parser anchors
> on printed labels matched as whole words: the blank surname row, present on most KRG cards, was
> skipped and shifted the mother's name into the surname slot on every such card.
>
> **Measured and not adopted.** `tessdata_best` models in the whole pipeline: names no better and
> slower (they did read cropped name strips better, which is recorded for a future zonal name
> read). **EasyOCR** (`fa` model) on the same strips: 2/19 exact names each, similarity 0.50 against
> Tesseract-best's ~0.45 — not worth PyTorch (~1 GB) in the offline image. Other layout modes gave
> more wrong names, not more right ones. **Names remain the weak field:** the engine returns
> near-miss letters (`مجمد` for `محمد`), which a lexicon of names already typed in the office's own
> records could snap — the next lever, not built.
...
| `<INSTITUTE>` | canonical institute label from the shared enum (§3.4); `custom_name` for out-of-city rows; **`General`** when the document has no institute (Step-1 client papers, a scanned case file — and, before UC-075/UC-118, the generated eligibility/compiled PDFs) | a **fixed canonical (romanized) label**, *not* the per-user UI translation — so the filename never changes with the viewer's language |
...
- **Generated PDFs** ~~(eligibility, compiled case) land in the same case folder with `input_source=system_generated`~~ — **no longer** (UC-075 for the letter, UC-118 for the compiled case): both are one-read job files under `GENERATED_ROOT` (§6.8), and the case folder holds only the papers the office filed. `input_source=system_generated` rows still exist on cases closed before those changes until `retire_compiled_exports` runs; they carry the paper's own Sorani name (`دۆسیەی کۆکراوەی کەیس`) and no institute.
...
│   │   └── LawyerNotes.tsx
│   ├── documents/               # DocumentUpload, ScanDocumentDialog, DocumentPreview, PageFrame
...
- **Generated-document panels** — `GeneratedDocumentPanel` is the shared shell for "the system produced this PDF": one button, three labels, and the start → poll-until-settled → show run. Each panel supplies what finishing *means* for it. Both outputs are now standalone job files (UC-075, UC-118) previewed from the job; `CompiledCasePanel` adds the auto-start off the mark-complete press, the Recompile button, and the listing of a `CompiledCase` the case still carries as a real document — a scanned case file from the backlog door (UC-114), or an export stored before UC-118 (§10.3).
...
- **The Processes list remembers its filters (UC-124)** — search, category, status, step and page are URL query parameters, replaced in place; a filter change resets the page. The row into a case passes the list URL as router state, and the case's back button returns there, so a visit to a case never blanks the list. Other list screens still hold their filters in component state.
- **Inline file previews share one frame (UC-123)** — `features/documents/PageFrame` is the iframe behind `DocumentPreview` (filed documents, the Step-1 letter, the compiled case) and the backlog door's preview: one page tall at whatever width it gets (`aspect-[2/3]`, A4 plus the viewer's toolbar), capped at 75 vh. A fixed height showed half a page; no cap ran off the screen.
...
- **Dates are entered through `DateField`, not `<input type="date">`** (2026-08-30, UC-108). A native date input takes its field order from a **setting outside the app** — the browser's UI language in Chrome and Edge, the Windows regional format in Firefox, the OS locale in Safari — so the office read month/day/year and nothing in this codebase could say otherwise. `components/ui/date-field.tsx` reads **day / month / year** on every machine and every browser, takes Arabic-Indic digits (folded by `lib/digits.ts`, shared with the PID box), keeps the three boxes in an LTR run inside RTL text, and carries a hand-built offline `Calendar`. Value in and out stays ISO `YYYY-MM-DD`, so it is a drop-in, and the native keyboard is kept whole — a box is selected entire on focus, ← → and Tab walk the three, ↑ ↓ step the value (day and month wrap, the year clamps). Left and right are **not** mirrored in RTL: the boxes are an LTR run in every language. **It also closes UC-072 at the source**: a change is emitted only when the boxes name a real day, so the year 2026 no longer passes through 2, 20 and 202 on its way to the server. **And each box refuses a number it can never hold at the keystroke** (UC-121): `segmentMax` caps the day at the real length of the month the other boxes name (else 31, or 29 for an open February), the month at 12 and the year at the window's top — while a day a later-typed month cannot hold is clamped to that month's last day, because the day is typed first and 31-then-February is a legitimate order. **The calendar is portalled to `<body>` and pinned to the viewport** (UC-122): under the box, or above it when that side has more room and below has too little, never past the top edge, following the box on scroll and resize — because as a child of the field it was cut by whatever ancestor clips, the step accordion first of all.
...
3. Offer **print** and **download**; ~~store the compiled file as a `Document(input_source="system_generated")` so it is reproducible and audited~~ — **superseded by UC-118 below**: the request is audited, the bytes are not kept.
...
> **Deviation (2026-08-31, the user — UC-118). The export is produced, not kept.** It is every
> paper on the case merged again behind a cover sheet, so storing it as a `Document` cost each
> closed case roughly twice its papers on disk — for a file the same papers reproduce in seconds.
> It is now written exactly as the Step-1 letter is (§6.8): a job file under
> `GENERATED_ROOT/compiled/`, previewed and printed off the mark-complete press, **collected on its
> first read**, the case's earlier export swept when a new one lands. The job row keeps
> `output_path` and the audit trail keeps the request (§11). `retire_compiled_exports` removes the
> copies stored before this rule, one-shot, after a backup — it reports first and writes only with
> `--apply`.
>
> **Two things a `CompiledCase` on a case can still be**, and the code tells them apart by
> `input_source`, never by type: an export stored before UC-118 (`system_generated`, retired by
> the command, excluded from the merge so it cannot nest) and **the paper case file the backlog
> door scanned in** (`imported`, UC-114) — the only copy of that case, which the command never
> reaches, `supersede_generated_documents` now filters out, and the merge **includes**, because it
