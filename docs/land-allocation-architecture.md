...
| **A taken national ID says whose it is** (2026-08-31 — UC-117) | §4.1: `pid` uniqueness is left to the `UniqueValidator` DRF generates from `ix_client_pid_active` | **`PidTakenValidator`** — DRF's own rule, kept (it is the **only** uniqueness check on the client-edit path, which has no service of its own), answering with **`errors.pid.taken:<holder>`**. `assert_pid_is_free` sends the same key, so all three doors agree. `lib/apiError.ts` splits the name off and interpolates it **bidi-isolated** (§9). See **§4.1** | The office read `"client with this pid already exists."`: English on screens that are otherwise Sorani, naming the *column*, and never saying **who** held the ID — the one fact the person typing needs. The good message existed in `clients/services.py` and was unreachable, shadowed by the generated validator. It matters now because the backlog run (§5.9) makes a collision routine rather than rare. **The one message carrying a runtime value**: the others stay parameterless because their bounds are constants, and a holder's name is not |
...
- **A national ID already on file names its holder** — `errors.pid.taken:<full name>`, the one validation message that carries a runtime value (UC-117). It is enforced by `PidTakenValidator`, which keeps the uniqueness rule DRF generates and replaces only what it says; `assert_pid_is_free` sends the same key from the service layer, so the client form, the scan confirm and the backlog form all answer alike. The frontend splits the name off and **bidi-isolates** it, because a Latin name inside a Sorani sentence reorders the words around it otherwise (§9).
