...
| **`GenerationJob` is not soft-deletable** | every domain model extends `SoftDeleteModel` (§3.1) | `GenerationJob` extends `TimeStampedModel` only. It records a *run*, not case data — the artefacts it produces (`Document`, and the audit row) are the durable, soft-deletable things. Nothing references a job as evidence | — (settled It.3) |
...
| **OCR pre-processing** | deskew · denoise · CLAHE · binarize — "where most accuracy is won" (§6.2) | **permanent deviation — measured, and it made things worse.** Thresholding a glossy card shredded the thin Arabic strokes: 10 good lines became 70 of noise. The raw image is fed as-is; cleanup is a per-image fallback, never a pipeline stage | — (settled It.5) |
...
| **Card scans** *(§6.5)* | `POST /api/v1/card-scans/` | **Stage a photographed ID** (multipart image or PDF) — **no client or case required**; converts to PDF, queues the read → `202` | Any authenticated |
...
It also decides where the file lives. The store path is `<CATEGORY>/<client_id>_<pid>/…_<person_name>_…pdf`: the folder is keyed by the PID and the name by the person, and **both are what the card supplies**. So a scan is *staged* on arrival and only filed once its reading is confirmed (§6.7).
...
flowchart LR
    subgraph BROWSER["Browser (either computer, offline)"]
        ...
        CAP --> UP["POST /card-scans/ (multipart image or PDF)"]
    end
...
**Image uploads are converted server-side** *(deviation from the above, settled It.5)*. A lawyer can photograph an ID today, before the It.6 capture screen exists, so `POST /documents/` and `POST /card-scans/` both accept JPEG/PNG/TIFF and convert to PDF on arrival — the store stays PDF-only and every downstream reader (preview, OCR, compile) handles exactly one format. No resampling: OCR accuracy depends on the original resolution. Alpha is flattened onto white, or a transparent PNG renders black. Client-side conversion still arrives with It.6; this is the path that works without it.
...
- **No pre-processing.** Grayscale + upscale + bilateral filter + adaptive threshold turned clean output into noise on a real card — 10 good lines became 70 junk ones, on all three models. A modern glossy ID photographed decently is *already* high-contrast, and thresholding shreds the thin Arabic strokes. The raw image is fed as-is; cleanup stays available as a per-image fallback to *try*, never a stage every scan passes through.
- **The MRZ is the trustworthy source.** The back carries an ICAO-9303 TD1 machine-readable zone, which `eng` reads near-perfectly and which is **self-verifying**: birth date, expiry and document number each carry a **check digit**, so a misread can be *detected* rather than merely hoped about. Dates, sex and the card number come from there; the Arabic front supplies only the Kurdish/Arabic name fields the MRZ transliterates away.
