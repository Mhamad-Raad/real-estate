...
- [x] **Bake `APP_VERSION`/`APP_BUILD` into the production images** *(done 2026-08-11: build args on both images, and the backup manifest records build + migration head)* as build args → env, and **record the build + applied migration head in the backup manifest**, so a restore can tell it is crossing a version boundary — *§2.6, §13.2*.
- [x] Dockerize the full stack + **offline image save/load** (`docker save`/`load`); Nginx serving static build + reverse-proxy; one-command bring-up — *§2.3*.
...
- [~] Full-disk encryption: **BitLocker** (Windows) / **FileVault** (macOS); encrypt the external drive — *§12, §2.5*. **Scoped down by the user 2026-08-12: the external drive only, later; the office PC stays unencrypted.** Steps and the recovery-key warning are in `docs/runbooks/hardening.md` §5. Dev Mac is on FileVault.
...
- [x] Security review, secrets handling, host hardening (firewall, no egress) — *§12*. **Done 2026-08-12.** Two findings, both fixed: (1) the append-only audit trail was documented in four places and **never built** — the app connects as the DB owner, so audit rows could be edited or deleted by anyone with the `.env` password; now enforced by trigger (`common/0003`, §12 records why a trigger and not the planned restricted role); (2) `measure_scale` created an Admin with a published password and nothing stopped either perf command running against the office database — both now refuse a non-scratch DB and seed unpassworded accounts. Clean: no Django admin, no secrets in git, `.env` never enters the bundle, CSP/headers correct, DB+Redis ports unpublished, backend non-root, `SECRET_KEY`/`ALLOWED_HOSTS` fail closed. Host checklist for the office: `docs/runbooks/hardening.md`.
