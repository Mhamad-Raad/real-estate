...
class GenerationJobViewSet(ReadOnlyModelViewSet):
    ...
    def file(self, request, pk=None):
        ...
        if not path.exists():
            # Ordinary, not a fault — every generated output is collected on its first read
        ...
        # and collecting on first read would delete the file out from under the download button
        # beside it. That is fixed on the client instead: the preview keeps the blob it fetched and
        # the download saves *that*, so one read now serves preview, print and download alike.
