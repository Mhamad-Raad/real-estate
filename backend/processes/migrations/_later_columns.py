...
Kept here rather than copied into each: the list was duplicated, and adding a field to one copy and
not the other left a fresh install unable to migrate at all (found adding `fast_entry`, UC-114).
Deferring a column that *does* already exist is harmless, so one always-growing list is correct for
every migration that reads it.

**Add every new `Process` field here.**
"""

LATER_COLUMNS = ("unique_code", "completed_by", "fast_entry")
